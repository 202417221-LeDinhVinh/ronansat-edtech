...
import { normalizeRenderableLatexSegments, tokenizeLatexSegments } from "@/utils/renderContent";
...
test("rejects inline math when the opener is followed by whitespace", () => {
  assert.deepEqual(tokenizeLatexSegments("$ 100$"), [{ type: "text", value: "$ 100$" }]);
});
...
test("does not turn sold quantities into currency when followed by count nouns", () => {
  assert.deepEqual(
    normalizeRenderableLatexSegments(
      tokenizeLatexSegments("During the first three days of a week, the store sold $128$ bottles of water."),
    ),
    [
      {
        type: "text",
